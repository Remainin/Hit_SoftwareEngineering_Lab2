...
public class updateAction extends ActionSupport {
	...
	public String upbook()  {
		try{
		if (author1.length() != 0 && book3.length() != 0 && book4.length() != 0 && book5.length() != 0) {
			...
			if (bookauthor.length() == 0) {
				...
				PreparedStatement stmt2 = con.prepareStatement("update book set AuthorID = ?,Publisher = ?, PublishDate = ?, Price = ?"+" where Title = ?");
				stmt2.setString(1, bookauthor);
				stmt2.setString(2, book3);
				stmt2.setString(3, book4);
				stmt2.setString(4, book5);
				stmt2.setString(5, book1);
				stmt2.executeUpdate();
				return SUCCESS;
			}
	...
	public String upauthor()  {
		try{
		if (bookauthor.length() != 0 && author2.length() != 0 && author3.length() != 0) {
			Connection con = GetConnection.get();
	...
	public String showjsp() {
		...
		book1 = new String(request.getParameter("name"));
